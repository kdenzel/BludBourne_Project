...
public abstract class PhysicsComponent implements Component {
    ...
    private static final String TAG = PhysicsComponent.class.getName();
    ...
    protected void initBoundingBox(float percentageWidthReduced, float percentageHeightReduced) {
        ...
        float origHeigth = Entity.FRAME_HEIGHT;

        //.8f for 20%
        float widthReductionAmount = 1 - percentageWidthReduced;
        float heightReductionAmount = 1 - percentageHeightReduced;

        if (widthReductionAmount > 0 && widthReductionAmount < 1) {
            width = widthReductionAmount;
        } else {
            width = Entity.FRAME_WIDTH;
        }

        if (heightReductionAmount > 0 && heightReductionAmount < 1) {
            height = heightReductionAmount;
        } else {
        ...
        switch (_boundingBoxLocation) {
            case BOTTOM_LEFT:
                ...
                break;
            case BOTTOM_CENTER:
                _boundingBox.setCenter(minX + origWidth / 2, minY + origHeigth / 4);
                break;
            case CENTER:
                _boundingBox.setCenter(minX + origWidth / 2, minY + origHeigth / 2);
                break;
        }
    }
